...
async function getSellerById(
  ...
  try {
    const res = await fetch(
      `https://nexcart-backend-o86x.onrender.com/admin/sellers/${id}`,
      {
        cache: "no-store",
        headers: {
...
export default async function SellerDetailPage({
  ...
  const totalValue =
    seller.products?.reduce((sum, p) => sum + p.price * p.quantity, 0) ?? 0;
  ...
  return (
    <div>
      ...
      <div className="mb-8 flex items-center gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-[#7a8a6a]">Seller #{seller.id}</p>
          </div>
        </div>
        {/* Back button */}
              ...
              {[
                {
                  icon: Store,
                  label: "Shop Name",
                  value: seller.shop.shopName,
                },
                {
                  icon: MapPin,
                  label: "Address",
                  value: seller.shop.shopAddress,
                },
                {
                  icon: FileText,
                  label: "Trade License",
                  value: seller.shop.tradeLicense,
                },
              ].map(({ icon: Icon, label, value }) => (
                <div
                  key={label}
      ...
      <div className="mt-6 rounded-2xl border border-[#e0d9cc] bg-white p-6 shadow-sm">
        <h2 className="mb-5 text-xl font-black text-[#1a1f16]">Products</h2>
