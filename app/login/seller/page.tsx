...
export default function SellerLoginPage() {
  ...
  const onSubmit = async (data: SellerLoginData) => {
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/seller/login`, data);
      ...
      const seller = response.data?.seller || response.data?.data;
      ...
      if (!seller) {
        toast.error("Seller data not found in login response");
        return;
      }

      // Clear old admin/customer/seller session cookies first
      Cookies.remove("token", { path: "/" });
      Cookies.remove("role", { path: "/" });
      Cookies.remove("seller", { path: "/" });

      // Save fresh seller session
      Cookies.set("token", token, {
        expires: 1,
        sameSite: "lax",
        path: "/",
      ...
      Cookies.set("seller", JSON.stringify(seller), {
        expires: 1,
        sameSite: "lax",
        path: "/",
      });
