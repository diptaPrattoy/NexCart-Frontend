    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="flex h-16 items-center gap-4">
          ...
          >
            <div className="relative flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-green-500 to-green-700 text-white shadow-sm shadow-green-600/30 transition-all duration-300 group-hover:scale-105 group-hover:-rotate-3 group-hover:shadow-lg group-hover:shadow-green-600/40">
              <ShoppingBag size={19} strokeWidth={2.4} />
            </div>
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.label}
                  href={item.href}
                  className={`relative rounded-lg px-3 py-2 text-[14.5px] font-semibold no-underline transition-colors duration-150 ${
                    active
                      ? "text-green-700"
                ...
                >
                  ...
                  {active && (
                    <span className="absolute left-3 right-3 -bottom-[1px] h-[2px] rounded-full bg-green-600" />
                  )}
                </Link>
              );
            })}
          ...
          <form
            ...
            className="relative hidden min-w-[220px] max-w-md flex-1 md:block"
          >
            <Search
              size={17}
            ...
            <input
              ...
              className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm font-medium text-slate-700 outline-none transition focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-100"
            />
          </form>
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2.5">
            {!token ? (
              <div className="hidden items-center gap-2.5 sm:flex">
                <Link
                  href="/login"
                  className="rounded-lg px-4 py-2 text-sm font-semibold text-slate-600 no-underline transition-colors duration-150 hover:bg-green-50 hover:text-green-700"
                >
                  Log in
                </Link>
                ...
                <Link
                  href="/register"
                  className="rounded-lg bg-green-600 px-4 py-2 text-sm font-semibold text-white no-underline shadow-sm shadow-green-600/25 transition-all duration-200 hover:-translate-y-0.5 hover:bg-green-700 hover:shadow-md hover:shadow-green-600/35 active:translate-y-0 active:scale-[0.98]"
                >
                  Sign up
                </Link>
              </div>
              ...
              <div className="hidden items-center gap-2.5 sm:flex">
                {role === "customer" && (
                  <Link
                    href="/dashboard/customer/?tab=cart"
                    aria-label={`Cart, ${cartCount} items`}
                    className="relative flex h-10 w-10 items-center justify-center rounded-lg bg-slate-100 text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:bg-green-50 hover:text-green-700"
                  >
                    <ShoppingCart size={18} strokeWidth={2.2} />

                    {cartCount > 0 && (
                      <span className="absolute -right-1.5 -top-1.5 flex h-[19px] min-w-[19px] items-center justify-center rounded-full bg-green-600 px-1 text-[11px] font-bold text-white ring-2 ring-white">
                        {cartCount}
                      </span>
                    )}
                  </Link>
                ...
                <button
                  onClick={goToDashboard}
                  className="group flex items-center gap-2 rounded-lg border border-transparent bg-slate-100 py-2 pl-2.5 pr-3.5 text-sm font-semibold text-slate-700 transition-all duration-200 hover:-translate-y-0.5 hover:border-green-200 hover:bg-green-50 hover:text-green-800 hover:shadow-sm hover:shadow-green-600/15 active:translate-y-0"
                >
                  <span className="flex h-6 w-6 items-center justify-center rounded-md bg-green-600 text-white transition-transform duration-200 group-hover:scale-110 group-hover:rotate-6">
                    <LayoutDashboard size={13} strokeWidth={2.4} />
                  </span>
                  Dashboard
                  {role && (
                    <span className="rounded-full border border-green-200 bg-white px-2 py-0.5 text-[11px] font-bold capitalize text-green-700 transition-colors duration-200 group-hover:bg-green-100">
                      {role}
                    </span>
                  )}
                </button>
                ...
                <button
                  onClick={handleLogout}
                  className="flex items-center gap-1.5 rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white transition-all duration-200 hover:-translate-y-0.5 hover:bg-red-700 hover:shadow-md hover:shadow-red-600/30 active:translate-y-0 active:scale-[0.98]"
                >
                  <LogOut size={15} strokeWidth={2.2} />
                  Logout
                </button>
            ...
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-lg text-slate-600 transition-colors duration-150 hover:bg-green-50 hover:text-green-700 lg:hidden"
              aria-label="Toggle menu"
            >
              {mobileOpen ? <X size={20} /> : <Menu size={20} />}
            </button>
      ...
      <div
        className={`overflow-hidden border-t border-slate-200/80 transition-[max-height,opacity] duration-300 ease-in-out lg:hidden ${
          mobileOpen ? "max-h-[34rem] opacity-100" : "max-h-0 opacity-0"
        }`}
      >
        <div className="flex flex-col gap-1 bg-white px-4 py-3">
          <form onSubmit={handleProductSearch} className="relative mb-2">
            <Search
              size={17}
              className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400"
            />
            ...
            <input
              ...
              onChange={(event) => setSearchTerm(event.target.value)}
              placeholder="Search products..."
              className="h-11 w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm font-medium text-slate-700 outline-none transition focus:border-green-500 focus:bg-white focus:ring-4 focus:ring-green-100"
            />
          </form>
          ...
          {navItems.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className={`rounded-lg px-3 py-2.5 text-[15px] font-semibold no-underline transition-colors duration-150 ${
                isActive(item.href)
          ...
          {!token ? (
            <div className="flex flex-col gap-2">
              <Link
                href="/login"
                className="rounded-lg border border-slate-300 px-4 py-2.5 text-center text-sm font-semibold text-slate-700 no-underline transition-colors duration-150 hover:border-green-600 hover:text-green-700"
              >
                Log in
              </Link>
              ...
              <Link
                href="/register"
                className="rounded-lg bg-green-600 px-4 py-2.5 text-center text-sm font-semibold text-white no-underline transition-colors duration-150 hover:bg-green-700"
              >
                Sign up
              </Link>
            </div>
            ...
            <div className="flex flex-col gap-2">
              {role === "customer" && (
                <Link
                  href="/dashboard/customer/?tab=cart"
                  className="flex items-center justify-between rounded-lg bg-green-50 px-4 py-2.5 text-sm font-semibold text-green-700 no-underline transition-colors duration-150 hover:bg-green-100"
                >
                  <span className="flex items-center gap-2">
                    <ShoppingCart size={16} /> Cart
                  </span>
                  ...
                  {cartCount > 0 && (
                    <span className="flex h-5 min-w-[20px] items-center justify-center rounded-full bg-green-600 px-1 text-xs font-bold text-white">
                      {cartCount}
                    </span>
                  )}
                </Link>
              ...
              <button
                onClick={goToDashboard}
                className="flex items-center gap-2 rounded-lg bg-slate-100 px-4 py-2.5 text-sm font-semibold text-slate-700 transition-colors duration-150 hover:bg-green-50 hover:text-green-700"
              >
                <LayoutDashboard size={15} />
                Dashboard
                {role && (
                  <span className="ml-auto rounded-md bg-green-100 px-2 py-0.5 text-xs font-bold capitalize text-green-700">
                    {role}
                  </span>
                )}
              </button>
              ...
              <button
                onClick={handleLogout}
                className="flex items-center justify-center gap-2 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors duration-150 hover:bg-red-700"
              >
                <LogOut size={15} />
                Logout
              </button>
